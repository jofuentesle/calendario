...
const $$Form = createComponent(
  ($$result, $$props, $$slots) => {
    ...
    return renderTemplate`${maybeRenderHead()}<form id="dynamicForm" method="POST" action="../../pages/api/send-quotes"> ${
      inputs &&
      inputs.map(
        ({ type = 'text', name, label = '', options = [], icons = null }) =>
          renderTemplate`<div class="mb-6 content-between"> <label${addAttribute(name, 'for')} class="block text-sm font-medium">${label}</label> ${
            type === 'select'
              ? renderTemplate`<select${addAttribute(name, 'name')}${addAttribute(name, 'id')} required class="py-3 px-4 block w-full text-md rounded-lg border border-gray-200"> <option value="" disabled selected>Selecciona una opción</option> ${options.map((option) => renderTemplate`<option${addAttribute(option.value, 'value')}>${option.label}</option>`)} </select>`
              : type === 'radio'
                ? // Lógica para Radio Buttons
                  options.map(
                    (option) =>
                      renderTemplate`<div class="inline-flex items-center mb-2"> <input type="radio"${addAttribute(option.value, 'id')}${addAttribute(name, 'name')}${addAttribute(option.value, 'value')} class="hidden" required> <label${addAttribute(option.value, 'for')} class="cursor-pointer"> <img${addAttribute(option.image, 'src')}${addAttribute(option.label, 'alt')} class="h-10 w-10 rounded-full border-2 border-gray-300"> <span class="text-sm font-medium text-gray-700">${option.label}</span> </label> </div>`
                  )
                : renderTemplate`<div class="relative"> ${icons && renderTemplate`<span class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"> <svg class="h-5 w-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"> ${icons} </svg> </span>`} <input${addAttribute(type, 'type')}${addAttribute(name, 'name')}${addAttribute(name, 'id')} required class="py-3 px-4 block w-full text-md rounded-lg border border-gray-200"${addAttribute(`Ingrese su ${label.toLowerCase()}`, 'placeholder')}> </div>`
          } </div>`
      )
  ...
  'C:/Users/Jordi/Documents/web/calendaris/src/components/ui/Form.astro',
  void 0
);
